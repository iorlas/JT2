...
LPCWSTR INIReadStr(LPCWSTR cat, LPCWSTR name, LPCWSTR fileName){
	...
	if(res == -2 || res == -3){
		LOG_WARNING_MF(L"ObjectTimer.cpp", 0, cat, name, L"INI Reader error: buffer too small");
		delete buff;
		return L"ERR1";
	}
	if(res == -1){
		res = GetLastError();
		if(res == 0x2){
			LOG_ERROR_MF(L"ObjectTimer.cpp", 0, L"INIReader", fileName, L"config file not found");
			delete buff;
			return L"ERR404";
		}
	}
