...
Core::Core(void) : DX9Manager(),
	...
	LOG_VERBOSE_MF(L"Core.cpp", 0, L"Core", L"Init", L"done");
}

Core::~Core(void){
	LOG_DEBUG_MF(L"Core.cpp", 0, L"Core", L"Deinit", L"unloading...");

	//Kill it with holy fire!
	for (vector<IRenderableObject*>::iterator it = renderObjects.begin(); it != renderObjects.end(); it++)
		delete *it;

	LOG_VERBOSE_MF(L"Core.cpp", 0, L"Core", L"Deinit", L"unloaded");
}

int Core::Init(void){
	LOG_VERBOSE_MF(L"Core.cpp", 0, L"Core", L"Init2", L"done");
	return TRUE;
}
...
bool Core::OnDXInitiated(void){
	...
	LOG_VERBOSE_MF(L"Core.cpp", 0, L"Core", L"Init3", L"waiting for the map");
	int tmp = 0;
	...
	do{
		...
		if((errCode = TryCopyMem(buff, (void*)LOL_MEM_MAP_NAME_OFFSET, 5)) != (int)buff){
			LOG_ERROR_CODE_MF(L"Core.cpp", 0, L"Core", L"Init3", L"cannot read map name memory, error code: ", errCode);
			SetStatusText(L"JT2: Cannot read map name memory, see log file.");
			return false;
		}
	}while((tmp = strlen(buff)) == 0);

	LOG_DEBUG_MF(L"Core.cpp", 0, L"Core", L"Objects", L"map found");
	...
	if(tmp == LOL_MAP_SUMMONERS_RIFT){
		...
		LOG_VERBOSE_MF(L"Core.cpp", 0, L"Core", L"Timers", L"Summoners Rift timers loaded");
	}

	//Indicators
	SetStatusText(L"JT2: Loading indicators");
	LOG_DEBUG_MF(L"Core.cpp", 0, L"Core", L"Indicators", L"creating...");
	INDICATOR_NEW(L"smite", 420, 25);
	INDICATOR_NEW(L"ignite", 50, 20);
	LOG_VERBOSE_MF(L"Core.cpp", 0, L"Core", L"Indicators", L"loaded");
	
	LOG_VERBOSE_MF(L"Core.cpp", 0, L"Core", L"Objects", L"creation is complete");
...
void Core::OnDXEndScene(LPDIRECT3DDEVICE9 pDevice){
	LOG_DEBUG_EF_MF(L"Core.cpp", 0, L"Core", L"Render", L"frame catched");
	...
	if((errCode = TryDirectReadMem<float>(&now, ingameClockNow)) != (int)&now){
		LOG_ERROR_CODE_MF(L"Core.cpp", 0, L"Core", L"Render", L"cannot read current time, error code: ", errCode);
		return;
	}
	if((errCode = TryDirectReadMem<float>(&start, ingameClockStart)) != (int)&start){
		LOG_ERROR_CODE_MF(L"Core.cpp", 0, L"Core", L"Render", L"cannot read start time, error code: ", errCode);
		return;
	}
	
	int curTime = (int)floor(now - start);
	LOG_DEBUG_EF_MF(L"Core.cpp", 0, L"Core", L"Render", L"got a time");

	if(curTime < 3){
		LOG_DEBUG_EF_MF(L"Core.cpp", 0, L"Core", L"Render", L"too early to render, skipping");
		return;
	}
	...
	LOG_DEBUG_EF_MF(L"Core.cpp", curTime, L"Core", L"Render", L"frame is done");
}
...
bool Core::OnDXFirstFrame(LPDIRECT3DDEVICE9 pDevice){
	LOG_DEBUG_MF(L"Core.cpp", 0, L"Core", L"FirstFrame", L"frame catched");
	if(!InitTimePointers()){
		SetStatusText(L"JT2: Cannot load clock memory, see log file.");
		LOG_DEBUG_MF(L"Core.cpp", 0, L"Core", L"FirstFrame", L"cannot read clock memory");
		return false;
	}
	...
	LOG_VERBOSE_MF(L"Core.cpp", 0, L"Core", L"FirstFrame", L"work is done");
	return true;
}

void Core::OnDXResetDevice(LPDIRECT3DDEVICE9 pDevice){
	LOG_DEBUG_MF(L"Core.cpp", 0, L"Core", L"DXReset", L"trying to reset DX resources...");
	for_each(vector<IRenderableObject*>, renderObjects, it)
		(*it)->OnResetDevice(pDevice);
	LOG_VERBOSE_MF(L"Core.cpp", 0, L"Core", L"DXReset", L"done");
}

void Core::OnDXLostDevice(){
	LOG_DEBUG_MF(L"Core.cpp", 0, L"Core", L"LostDevice", L"trying to free DX resources...");
	for_each(vector<IRenderableObject*>, renderObjects, it)
		(*it)->OnLostDevice();
	LOG_VERBOSE_MF(L"Core.cpp", 0, L"Core", L"LostDevice", L"done");
}

bool Core::InitTimePointers(){
	LOG_DEBUG_MF(L"Core.cpp", 0, L"Core", L"Pointers", L"loading...");
	int errCode = 0;
	int tmp = 0;
	...
	if(ingameClockNow == NULL){
		...
		if((errCode = TryCopyMem(&tmp, (void*)LOL_MEM_CURRENT_TIME_STRUCT_PTR, sizeof(void*))) != (int)&tmp){
			LOG_ERROR_CODE_MF(L"Core.cpp", 0, L"Core", L"Pointers", L"cannot read in-game now clock memory, error code: ", errCode);
			SetStatusText(L"JT2: Cannot read in-game now clock memory, see log file.");
			return false;
		}
		if(tmp == 0x0){
			LOG_ERROR_MF(L"Core.cpp", 0, L"Core", L"Pointers", L"cannot read in-game now clock memory: struct pointer is null");
			SetStatusText(L"JT2: Cannot read in-game now clock memory, see log file.");
			return false;
		}
		...
		LOG_VERBOSE_MF(L"Core.cpp", 0, L"Core", L"Pointers", buf);

		//Check value
		float tmp2 = .0f;
		if((errCode = TryCopyMem(&tmp2, (void*)ingameClockNow, sizeof(float))) != (int)&tmp2){
			LOG_ERROR_CODE_MF(L"Core.cpp", 0, L"Core", L"Pointers", L"cannot check in-game now clock memory, error code: ", errCode);
			SetStatusText(L"JT2: Cannot read in-game now clock memory, see log file.");
			return false;
		}
		...
		if(tmp2 < -1 || tmp2 > 14400){
			SetStatusText(L"JT2: Cannot read in-game now clock memory, see log file.");
			LOG_ERROR_MF(L"Core.cpp", 0, L"Core", L"Pointers", L"it seems, 'now clock' address is broken(< -1 or > 14400)");
			return false;
		}
	}
	...
	if(ingameClockStart == NULL){
		...
		if((errCode = TryCopyMem(&tmp2, (void*)ingameClockStart, sizeof(float))) != (int)&tmp2){
			LOG_ERROR_CODE_MF(L"Core.cpp", 0, L"Core", L"Pointers", L"cannot check in-game start clock memory, error code: ", errCode);
		...
		if(tmp2 < -1 || tmp2 > 14400){
			SetStatusText(L"JT2: Cannot read in-game start clock memory, see log file.");
			LOG_ERROR_MF(L"Core.cpp", 0, L"Core", L"Pointers", L"it seems, 'start clock' address is broken(< -1 or > 14400)");
			return false;
		}
	}

	LOG_VERBOSE_MF(L"Core.cpp", 0, L"Core", L"Pointers", L"pointers are found");
	return true;
}
